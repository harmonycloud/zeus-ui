		...
		return (
			<Actions>
						...
						if (record.managePlatformAddress === '') {
							Message.show(
								messageConfig(
									'error',
									'失败',
									`请先到服务暴露中，暴露${record.name}的${record.name}${record.type === 'elasticsearch' ? '-kibana' : record.type === 'kafka' ? '-manager-svc' : '-console-svc'}服务。`
								)
							);
							return;
						}
				...
				<LinkButton
					onClick={() =>
						history.push(
							`/serviceList/serverVersion/${record.type}/${record.name}/${record.aliasName}`
						)
					}
				>
					版本管理
